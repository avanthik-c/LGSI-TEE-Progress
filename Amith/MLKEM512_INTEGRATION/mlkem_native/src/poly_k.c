...
void mlk_poly_getnoise_eta1_4x(mlk_poly *r0, mlk_poly *r1, mlk_poly *r2,
                               ...
                               uint8_t nonce3)
{
    #define BUF_ROW_SIZE MLK_ALIGN_UP(MLKEM_ETA1 * MLKEM_N / 4)
    #define EXTKEY_ROW_SIZE MLK_ALIGN_UP(MLKEM_SYMBYTES + 1)

    /* C90 Strict: Declarations at the top */
    /*
    uint8_t *buf_flat;
    uint8_t *extkey_flat;
    uint8_t *buf[4];
    ...
    extkey_flat = memalign(16, 4 * EXTKEY_ROW_SIZE);

    if (!buf_flat || !extkey_flat) {
        free(buf_flat);
        free(extkey_flat);
        return;
    }
    ...
    extkey[0] = extkey_flat;
    extkey[1] = extkey_flat + EXTKEY_ROW_SIZE;
    extkey[2] = extkey_flat + 2 * EXTKEY_ROW_SIZE;
    extkey[3] = extkey_flat + 3 * EXTKEY_ROW_SIZE;
    */

    MLK_ALIGN uint8_t buf[4][BUF_ROW_SIZE];
    MLK_ALIGN uint8_t extkey[4][EXTKEY_ROW_SIZE];
    ...
    mlk_zeroize(extkey_flat, 4 * EXTKEY_ROW_SIZE);
    free(buf_flat);
    free(extkey_flat);
    */
